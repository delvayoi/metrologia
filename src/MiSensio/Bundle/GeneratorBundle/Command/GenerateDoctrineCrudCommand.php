...
use MiSensio\Bundle\GeneratorBundle\Generator\DoctrineCrudGenerator;
use MiSensio\Bundle\GeneratorBundle\Generator\DoctrineFormGenerator;
use MiSensio\Bundle\GeneratorBundle\Manipulator\RoutingManipulator;
/**
    ...
    protected function configure()
    {
        $this
            ->setDefinition(array(
                ...
                new InputOption('paginator', '', InputOption::VALUE_NONE, 'quieres crear paginado en las listas(yes/no)'),
                new InputOption('buscador', '', InputOption::VALUE_NONE, 'quieres crear un buscador en las listas(yes/no)'),
                new InputOption('campo', '', InputOption::VALUE_REQUIRED, 'entre el campo por el que se va a buscar'),
            ))
            ->setDescription('Generates a CRUD con sus datos de forma tableada')
            ->setHelp(<<<EOT
...
EOT
            ...
            ->setName('MiSensio:generate:Crud')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $paginator = $input->getOption('paginator');
        $buscador = $input->getOption('buscador');
        $campo = $input->getOption('campo');
        ...
        $generator->generate($bundle, $entity, $metadata[0], $format, $prefix, $withWrite, $forceOverwrite, $paginator, $buscador, $campo);
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        $question->setValidator(array('MiSensio\Bundle\GeneratorBundle\Command\Validators', 'validateEntityName'));
        ...
        $input->setOption('route-prefix', $prefix);
         // hecho por gilberto y Delva 
         // paginador
       $paginator = $input->getOption('paginator') ?: false;
       $output->writeln(array(
            '',
            'por defecto saldra sin paginado en las listas.',
            'si desea agregarlo escoja (yes).',
            '',
        ));    
        $question = new ConfirmationQuestion($questionHelper->getQuestion('Do you want to generate the "paginador" actions', $paginator ? 'yes' : 'no', '?', $paginator), $paginator);
        $paginator = $questionHelper->ask($input, $output, $question);       
        $input->setOption('paginator', $paginator);            
        
        $buscador = $input->getOption('buscador') ?: false;
        $output->writeln(array(
            '',
            'por defecto saldra sin buscador en las listas.',
            'si desea agregarlo escoja (yes).',
            '',
        ));
         $question = new Question($questionHelper->getQuestion('You want to create finder', $buscador ? 'yes' : 'no', '?' , $buscador),  $buscador );
          $buscador = $questionHelper->ask($input, $output, $question); 
         $input->setOption('buscador', $buscador);        
            if($input->getOption('buscador'))
            {
                $campo = $input->getOption('campo');
                $output->writeln(array(
                    '',
                    'Entre el campo por el que se va a buscar.',
                    '',
                ));
                $question = new Question($questionHelper->getQuestion('Entre el campo '  ,$campo), $campo);
                $question->setValidator(array('MiSensio\Bundle\GeneratorBundle\Command\Validators', 'validateCampo'));
                  $campo = $questionHelper->ask($input, $output, $question);
      
                //$campo = $questionHelper->doAsk($output, $questionHelper->getQuestion('Entre el campo por el que se va a buscar', $campo), array('MiSensio\Bundle\GeneratorBundle\Command\Validators', 'validateCampo'), false, $campo);
               // $campo= $questionHelper->ask($output ,$questionHelper->getQuestion('Entre el campo por el que se va a buscar', $campo), array('MiSensio\Bundle\GeneratorBundle\Command\Validators', 'validateCampo'), false, $campo);
                
                $input->setOption('campo', $campo);
            }       
        // summary
        $output->writeln(array(
            '',
